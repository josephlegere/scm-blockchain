...
let DesignMachine = class {

    constructor() {

        this.page_container_title = 'design-machine';
    ...
    triggers() {
        ...
        let trigger_click_function = async (e) => {
            ...
            if (card) {
                ...
                if (collectionItem) {
                    this.acceptRequest(this.designs[_index]);
                }
            }
        }
    ...
    render() {
        ...
        _html = `
            <div class="page-container" id="${this.page_container_title}">
                ...
                <div class="row left-align" id="design-list">
                </div>
            </div>
        `;
                ...
                <div class="col s12 m6">
                    <div class="card" data-unique="${elem._id}" data-index="${key}">
                        <div class="card-content"> <!-- --------------CARD CONTENT-------------- -->
                            <span class="card-title">Design <b>${key + 1}</b> for ${elem.manufacturer.name}</span>
                            <span class=""><b>Status</b></span>
                            <ul class="collection">
                                <li class="collection-item">
                                    <div class="row">
                                        <div class="col s6">
                                            Send Quote for Design
                                        </div>
                                    </div>
                                </li>
                            </ul>
    ...
    getDesigns() {
        this.fetchDesigns()
            .then(res => {
                console.log(res)
                let _designs = res.data;

                if (!res.success) throw res.error;
                if (res.count < 1) throw 'No designs are listed yet.';

                this.designs = _designs;
                this.renderList(_designs, '#design-list');
            })
            .catch(err => {
                let _html = '';
                console.log(err);
    ...
    getDocument(uniq, type) {
        let url = SERVER_ATTR.PAGE_DESIGNS + `/${type}/${uniq}`;
    ...
    //controllers
    async fetchDesigns() { //fetch 
        let userLogged = localDB.get(['log_token']);
        const sendRequest = new Request(SERVER_ATTR.PAGE_DESIGNS, {
            method: 'GET',
            //body: body,
            headers: {
